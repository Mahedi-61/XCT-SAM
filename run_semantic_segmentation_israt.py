...
def plot_output(test_df_pred, test_df_label, output, args):
    ...
    for img_path, label_path, pred_mask in zip(
        ...
        # Minimum connected-component area filter
        # Set --min_area 0 to disable (test3/test5/test6).
        # test4: --min_area 75 removes ~85 tiny FPs per image; smallest real defect = 79px.
        if args.min_area > 0:
        ...
        plt.savefig(
            os.path.join(args.output_dir, f"mask_{os.path.basename(img_path)}"),
...
CUDA_VISIBLE_DEVICES=0 python3 run_semantic_segmentation_israt.py \
  --dataset_dir ./datasets --output_dir ./output/israt/test6 --ckpt_path AutogluonModels/Dloss/class_2 --data_name test6_class1 --batch_size 4 \
  --threshold 0.50 --morph dilate --morph_size 1 --min_area 0
"""
