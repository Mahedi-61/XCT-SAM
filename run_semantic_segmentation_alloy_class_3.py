...
# ---------------------------------------------------------------------------
# Stage 2: XCT domain fine-tuning for class_3 inclusions, warm-started from alloy Stage-1 checkpoint.
#
# Changes vs. run_semantic_segmentation_class_3.py (cold-start):
...
#   3. max_epoch=20 (vs 20): warm start converges faster on XCT train set.
#   4. patience=10: allow early stopping once XCT val-IoU plateaus.
...
ALLOY_CKPT_DEFAULT = "AutogluonModels/alloy/class_3"
XCT_CKPT_DEFAULT   = "AutogluonModels/alloy_to_xct/class_3"
...
def get_valid_region(img_arr):
    ...
    # preventing dark background pixels from being included when 10% of max is
    # too low (e.g. test5 where background isn't very dark).
    if mask.mean() > 0.78:
    ...
    # Convex hull prevents concave notches from excluding interior defect sites.
    hull = cv2.convexHull(cnt)
    metal_mask = np.zeros(img_arr.shape[:2], dtype=np.uint8)
    cv2.drawContours(metal_mask, [hull], -1, 1, thickness=cv2.FILLED)
...
def plot_output(test_df_pred, test_df_label, output, args):
    ...
    for img_path, label_path, pred_mask in zip(
        ...
        binary_mask = ndi.binary_opening(binary_mask, structure=np.ones((1, 1))).astype(np.uint8)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Stage 2: XCT inclusion fine-tuning warm-started from alloy Stage-1 checkpoint."
    )
    ...
    parser.add_argument("--output_dir",         type=str, default="output/alloy_to_xct/class_3")
    parser.add_argument("--data_name",          type=str, default="train_class3")
    ...
    parser.add_argument("--ckpt_path",          type=str, default=XCT_CKPT_DEFAULT,
                        help="Base path for checkpoints. Best model saved to "
                             "Path where the trained checkpoint is saved/loaded.")
    ...
    val_data = "train_" + args.data_name.split("_")[1]
    val_df   = expand_path(
        pd.read_csv(os.path.join(dataset_dir, f"{val_data}.csv")), dataset_dir
    )
...
# CUDA_VISIBLE_DEVICES=0,1 python3 run_semantic_segmentation_alloy_class_3.py \
#   --dataset_dir datasets/gan-generated \
#   --data_name   train_class3 \
#   --output_dir  output/alloy_to_xct/class_3/train \
#   --alloy_ckpt_path AutogluonModels/alloy/class_3 \
#   --ckpt_path   AutogluonModels/alloy_to_xct/class_3 \
#   --num_gpus 2 --batch_size 8 --rank 2

# --- Evaluate Stage-2 model ---
# CUDA_VISIBLE_DEVICES=0 python3 run_semantic_segmentation_alloy_class_3.py \
#   --dataset_dir datasets/gan-generated \
#   --ckpt_path   AutogluonModels/alloy_to_xct/class_3 \
#   --output_dir  output/gan_adapt/test6_c3 \
#   --data_name   test6_class3 \
