...
def get_valid_region(img_arr):
    ...
    # Fixed or percentage-based thresholds fail for dark images (e.g. max=72)
    # where metal and background have very different intensities per image.
...
def plot_output(test_df_pred, test_df_label, output, args):
    ...
    for img_path, label_path, pred_mask in zip(test_df_pred['image'], test_df_label['label'], output):
        ...
        # Threshold based on intensity midpoint
        mask_arr = np.array(pred_mask).squeeze().astype(np.float32)
        threshold = 0.5 #np.mean(mask_arr)
        binary_mask = (mask_arr > threshold).astype(np.uint8)  # defects=1, surface=0

        # Remove isolated single-pixel noise — keep only connected components with at least 2 pixels. 
        # Prevents 1-pixel FPs from turning IOU=1.0 → 0.0 on images that should be empty predictions.
        ...
        # plt.subplot(1, 4, 3)
        # plt.title("Input Label (Gen.)")
        # plt.imshow(display_img_mask, cmap='gray')
        # plt.axis("off")
...
if __name__ == "__main__":
    ...
    val_data = "test6_" + args.data_name.split("_")[1]
...
"""
python3 run_semantic_segmentation_class_2.py --task gan-generated --dataset_dir ./datasets --output_dir ./output/class_2/train --data_name train_class2 --num_gpus 2 --batch_size 8 --rank 2
"""

"""
CUDA_VISIBLE_DEVICES=0 python3 run_semantic_segmentation_class_2.py --task gan-generated --dataset_dir ./datasets --output_dir ./output/gan/test1_c2 --ckpt_path AutogluonModels/class_2 --data_name test1_class2 --num_gpus 1 --batch_size 4 --rank 2 --eval
"""
