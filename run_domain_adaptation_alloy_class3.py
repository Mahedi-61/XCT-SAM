...
# ---------------------------------------------------------------------------
# Alloy dataset — class_3 intermediate domain adaptation  (Stage 1)
# (bright inclusion defects on dark alloy surface, P92HBN specimens)
#
# Differences vs. run_domain_adaptation_alloy_class2.py:
#   1. Targets bright inclusions (class_3) instead of dark pores (class_2).
#   2. All CSV paths reference class_3 (train_class3.csv, val_class3.csv).
#   3. Default checkpoint paths use class_3 subdirectories.
#   4. Same full-frame valid region (alloy images have no circular border).
#   5. Same DiceFocalLoss + 25 epoch schedule as class_2 alloy adaptation.
# ---------------------------------------------------------------------------
MAX_EPOCH = 25
LR        = 2e-4
...
def plot_output(test_df_pred, test_df_label, output, args):
    ...
    for img_path, label_path, pred_mask in zip(
        ...
        plt.subplot(1, 3, 1)
        plt.title("Input Image");  plt.imshow(img_arr, cmap="gray");  plt.axis("off")
        plt.subplot(1, 3, 2)
        plt.title("Ground Truth"); plt.imshow(label_arr, cmap="gray"); plt.axis("off")
        plt.subplot(1, 3, 3)
        plt.title(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Alloy class_3 intermediate domain adaptation (Stage 1)"
    )
    ...
    parser.add_argument("--output_dir",         type=str,   default="output/alloy/class3")
    parser.add_argument("--data_name",          type=str,   default="val_class3",
                        help="CSV stem to evaluate on after training.")
    parser.add_argument("--ckpt_path",          type=str,   default="AutogluonModels/alloy/class_3",
                        help="Path to save (train) or load (--eval) the best checkpoint.")
    parser.add_argument("--ckpt_path_last",     type=str,   default="AutogluonModels/alloy/class_3_last",
                        help="Path to save the last-epoch model after training.")
    parser.add_argument("--per_gpu_batch_size", type=int,   default=1)
    parser.add_argument("--batch_size",         type=int,   default=4)
    # post-processing knobs
    parser.add_argument("--threshold",  type=float, default=0.50,
                        help="Sigmoid threshold for binarising predicted mask.")
    parser.add_argument("--morph",      type=str,   default="open",
                        choices=["none", "open", "close", "dilate"],
    ...
    train_df = expand_path(
        pd.read_csv(os.path.join(dataset_dir, "train_class3.csv")), dataset_dir
    )
    val_df = expand_path(
        pd.read_csv(os.path.join(dataset_dir, "val_class3.csv")), dataset_dir
    )
...
# --- Train (Stage 1 alloy intermediate domain adaptation) ---
# Best  → AutogluonModels/alloy/class_3
# Last  → AutogluonModels/alloy/class_3_last
#
# CUDA_VISIBLE_DEVICES=0,1 python3 run_domain_adaptation_alloy_class3.py \
#     --dataset_dir datasets/alloy \
#     --output_dir  output/alloy/class3/train \
#     --data_name   train_class3.csv \
#     --num_gpus 2 --batch_size 8 --rank 2

# --- Evaluate best checkpoint ---
# CUDA_VISIBLE_DEVICES=0 python3 run_domain_adaptation_alloy_class3.py \
#     --dataset_dir datasets/alloy \
#     --ckpt_path   AutogluonModels/alloy/class_3 \
#     --output_dir  output/alloy/class3/val \
#     --data_name   val_class3 \
#     --num_gpus 1 --morph open --morph_size 3 --eval